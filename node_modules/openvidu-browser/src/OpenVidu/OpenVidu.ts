...
import { RTCPeerConnection as ReactNativeRTCPeerConnection,
...
export class OpenVidu {
  ...
  constructor() {
    ...
    if (this.platform.isReactNative) {
      window.btoa = btoa;
      window['RTCPeerConnection'] = ReactNativeRTCPeerConnection;
      window['RTCIceCandidate'] = ReactNativeRTCIceCandidate;
      window['RTCSessionDescription'] = ReactNativeRTCSessionDescription;
      window['MediaStream'] = ReactNativeMediaStream;
      window['MediaStreamTrack'] = ReactNativeMediaStreamTrack;
      (<any>window.navigator)['mediaDevices'] = ReactNativemediaDevices;
      //window['getUserMedia'] = ReactNativegetUserMedia;
    }

    if (this.platform.os!!.family === 'iOS' || this.platform.os!!.family === 'Android') {
            ...
            const getNewVideoDimensions = (): Promise<{ newWidth: number, newHeight: number }> => {
              return new Promise((resolve, reject) => {
                if (this.platform['isIonicIos']) {
                  ...
                  const newWidth = <number>((this.platform.name!!.toLowerCase().indexOf('firefox') !== -1) ? firefoxSettings.width : publisher.videoReference.videoWidth);
                  const newHeight = <number>((this.platform.name!!.toLowerCase().indexOf('firefox') !== -1) ? firefoxSettings.height : publisher.videoReference.videoHeight);
  ...
  checkSystemRequirements(): number {
    const browser = this.platform.name;
    const family = this.platform.os!!.family;
    const userAgent = !!this.platform.ua ? this.platform.ua : navigator.userAgent;
    const isReactNative = this.platform['isReactNative'];
  ...
  checkScreenSharingCapabilities(): number {
    const browser = this.platform.name;
    const version = this.platform?.version ? parseFloat(this.platform.version) : -1;
    const family = this.platform.os!!.family;
  ...
  getDevices(): Promise<Device[]> {
    return new Promise<Device[]>((resolve, reject) => {
      navigator.mediaDevices.enumerateDevices().then((deviceInfos) => {
        ...
        if (this.platform['isIonicAndroid'] && cordova.plugins && cordova.plugins.EnumerateDevicesPlugin) {
        ...
        } else if(this.platform['isReactNative']) {
        ...
        if (!!myConstraints.videoTrack && !!myConstraints.audioTrack ||
          ...
          if (typeof options.videoSource === 'string') {
            ...
            if (options.videoSource === 'screen' ||
              ...
              (this.platform.name === 'Electron' && options.videoSource.startsWith('screen:'))) {
              ...
              if (navigator.mediaDevices['getDisplayMedia'] && this.platform.name !== 'Electron') {
      ...
      if (typeof videoSource === 'string') {
        ...
        if (!this.isScreenShare(videoSource)) {
          ...
          if (platform['isReactNative']) {
            (<MediaTrackConstraints>myConstraints.constraints!.video)['facingMode'] = videoSource === '0' ? 'environment' : 'user';
          } else {
          ...
          if (!this.checkScreenSharingCapabilities()) {
            const error = new OpenViduError(OpenViduErrorName.SCREEN_SHARING_NOT_SUPPORTED, 'You can only screen share in desktop Chrome, Firefox, Opera, Safari (>=13.0) or Electron. Detected client: ' + this.platform.name);
            logger.error(error);
            reject(error);
          } else {

            if (this.platform.name === 'Electron') {
              const prefix = "screen:";
              ...
              if (!!this.advancedConfiguration.screenShareChromeExtension && !(this.platform.name!.indexOf('Firefox') !== -1) && !navigator.mediaDevices['getDisplayMedia']) {
                ...
                if (navigator.mediaDevices['getDisplayMedia']) {
                  ...
                  const firefoxString = this.platform.name!.indexOf('Firefox') !== -1 ? publisherProperties.videoSource : undefined;
  ...
  private isScreenShare(videoSource: string) {
    return videoSource === 'screen' ||
      videoSource === 'window' ||
      (this.platform.name === 'Electron' && videoSource.startsWith('screen:'))
  }

  private isIPhoneOrIPad(userAgent): boolean {
    const isTouchable = typeof document !== 'undefined' && 'ontouchend' in document;
