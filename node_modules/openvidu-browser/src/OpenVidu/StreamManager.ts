...
export class StreamManager extends EventDispatcher {
    ...
    constructor(stream: Stream, targetElement?: HTMLElement | string) {
        ...
        if (!!targetElement) {
            ...
            if (!!targEl) {
                ...
                if (this.stream.session.openvidu.platform.name === 'Safari') {
    ...
    addVideoElement(video: HTMLVideoElement): number {
        ...
        if (platform['isReactNative']) {
            if (!!this.stream.getMediaStream() && !!video ) {
                (<RTCView>video).setNativeProps({streamURL: (<any>this.stream.getMediaStream()).toURL()});
            }
        ...
        this.videos.forEach(streamManagerVideo => {
            ...
            if (platform['isReactNative']) {
                if (!!streamManagerVideo.video) {
                    (<RTCView>streamManagerVideo.video).setNativeProps({streamURL: null});
                }
            } else {
    ...
    updateMediaStream(mediaStream: MediaStream) {
        this.videos.forEach(streamManagerVideo => {
            ...
            if (this.stream.session.openvidu.platform['isIonicIos']) {
            ...
            if (platform['isReactNative']) {
                (<RTCView>streamManagerVideo.video).setNativeProps({streamURL: (<any>mediaStream).toURL()});
            }
        });
    }
